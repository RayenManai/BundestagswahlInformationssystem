...
CREATE TABLE Erststimme (
    StimmzettelId integer primary key,
    kandidatId integer references Direktkandidat check (
        exists (
            select * 
            from Direktkandidat d, Stimmzettel s 
            where kandidatId = d.DKId 
                  and s.StimmzettelId = StimmzettelId 
                  and d.wahlkreisId = s.wahlkreisId
            )
        ),
)
